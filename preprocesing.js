...
async function loadModel() {
    ...
    // const myFavoriteNumber = 888;
    // alert('Hello! My favorite number is ' + myFavoriteNumber)
...
function predictImage() {
    ...
    if (height > width){
        height =20;
        const scalFactor = image.rows/ height;
        width = Math.round(image.cols/scalFactor);
    }
    else{
        width =20;
        const scalFactor = image.cols/ width;
        height = Math.round(image.rows/scalFactor);
    }
    ...
    //vizualizare binara a pixelilor inainte de normalizare\\
    ...
    //creza predict metoda din canvas\\
